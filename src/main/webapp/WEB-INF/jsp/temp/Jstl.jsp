	...
	<c:choose>
		...
		<c:when test="${step eq '2'}">
			<li>
				...
				<h4>기본문법 : &#60;c:import url="URL주소 값" char Encoding="utf-8"/&#62;</h4>
		
				<c:import url="/temp/jstlImport.do" charEncoding="utf-8"/>
				<%-- <c:import url="/temp/selectList.co" charEncoding="utf-8"/> --%>
			</li>
		</c:when>
		...
		<c:when test="${step eq '10'}">
			<li>
				...
				<%-- 결과값은 6 나옴, 0,1,2,3,4,5,6 7번째에서 시작한다는 뜻, 공백도 문자로 취급 --%>
			</li>
		</c:when>
		
		<c:when test="${step eq '11'}">
			<li>
				<h3>fn:starsWith() : 주어진 문자열이 특정 문자열 값으로 시작되는지 확인하는 데 사용</h3>
				
				<c:set var="String" value="Welcome to JSP programming"/>
				<c:if test="${fn:startsWith(String, 'Welcome')}">
					<p>String starts with Welcome</p>
				</c:if>
				<%-- 주어진 문장은  Welcome to JSP programming 이고, String문자열 Welcome으로 시작하므로 
				하단에 p태그로 추가한 String starts with Welcome 출력--%>
				
				
				<c:if test="${fn:startsWith(String, 'programming')}">
					<p>String starts with programming</p>
				</c:if>
				<%-- 주어진 문장은  Welcome to JSP programming 이고, String문자열 programming으로 시작하지 않으므로 
				하단에 p태그로 추가한 String starts with programming 출력안함--%>
			</li>
		</c:when>
		...
		<c:when test="${step eq '14'}">
			<li>
				...
				<%-- 문자열도 되고 리스트도 가능하다. 문자열 -> 문자수, 리스트 -> 항목의 수 --%>
				<c:set var="str1" value="This is forst string"/>
				<c:set var="str" value="1,2,3,4,5,6"/>
				<c:set var="strSplit" value="${fn:split(str,',')}"/>
		...
		<c:when test="${step eq '17'}">
			<li>
				...
				<c:out value="${fn:substring(string, 0, 4)}"/> <%-- 0,1,2,3,4 번째만... 2023()  --%>
				<c:out value="${fn:substring(string, 5 ,7)}"/> <%-- 5,6,7번째만... 04()  --%>
			</li>
		</c:when>
		
		<c:when test="${step eq '18'}">
			<li>
				<h3>fn:replace() : 모든 문자열을 다른 문자열 시퀸스로 바꿈</h3>
				
				<c:set var="string" value="2023-04-19"/>
				<c:out value="${fn:replace(string, '-', '.')}"/> <%-- string 문자열의 -를 .으로 변경 --%>
			</li>
		</c:when>
		<%-- JAVA에서 replace는 앞에 있는거 1개만 바뀜, jsp의 replace와 같은 효과를 보려면 replaceAll을 사용해야 됨. --%>
